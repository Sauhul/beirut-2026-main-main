export function getCorrectCategory(productName: string): string {
  ...
  if (
    ...
    name.includes("pan") ||
    name.includes("baklawa") ||
    name.includes("galletas") ||
    name.includes("turrones")
  )
    return "panaderia-y-dulces";
  if (
    name.includes("tahine") ||
    name.includes("labne") ||
    name.includes("queso") ||
    name.includes("aceitunas") ||
    name.includes("chanclis")
  )
    return "lacteos-y-aderezos";
  if (
